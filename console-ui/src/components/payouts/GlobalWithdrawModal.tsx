  ...
  return (
    <div className="px-6 pb-6">
      ...
      <p className="text-sm text-text-secondary mb-5 flex items-center gap-2">
        <Building2 size={15} />
        {status.destination_last4 ? `Bank account ending in ${status.destination_last4}` : "Your verified bank account"}
      </p>
      ...
      {value > balance && <p role="alert" className="text-sm text-coral mb-4">This amount exceeds your available earnings.</p>}
      ...
      <div className="flex gap-3">
        <button onClick={onCancel} disabled={loading} className="flex-1 py-3 rounded-lg border border-border-dim text-sm font-bold disabled:opacity-50">Cancel</button>
        <button onClick={onConfirm} disabled={loading || !valid} className="flex-1 py-3 rounded-lg bg-teal text-white text-sm font-bold flex items-center justify-center gap-2 disabled:opacity-50">
