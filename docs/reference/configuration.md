...
| `EIGENINFERENCE_STRIPE_GLOBAL_PAYOUTS_ENABLED` | bool | `false` | `coordinator/billing/config.go` (`ReadConfig`, `Check`) | Enables new international onboarding, quotes and withdrawals. Reconciliation continues with configured credentials even when disabled. |
...
| `EIGENINFERENCE_STRIPE_GLOBAL_PAYOUTS_SECRET_KEY` | secret | falls back to `EIGENINFERENCE_STRIPE_SECRET_KEY` | `coordinator/billing/config.go` (`ReadConfig`) | Restricted API key with Global Payouts permissions. |
